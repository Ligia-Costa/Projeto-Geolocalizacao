...
async function obterClima(latitude, longitude) {
    const apiKey = '5dd17334cb954cf899c90059250104'; // Minha chave da API
    ...
    try {
        ...
        if (dados.error) {
            console.error("Erro ao obter a temperatura:", dados.error.message);
            return;
        }

        // Atualizando os elementos no HTML
        document.getElementById("temp").textContent = dados.current.temp_c;
        document.getElementById("condicao").textContent = dados.current.condition.text;
        document.getElementById("chuva").textContent = dados.current.precip_mm;
        document.getElementById("icone-clima").src = "https:" + dados.current.condition.icon;

        // Tornar visível a seção do clima
